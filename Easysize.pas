...
begin
     ...
     RegisterComponents('GpSi', [TFormResizer]);
end;
     ...
     for I := 0 to Owner.ComponentCount - 1 do
      begin
         ...
         if Owner.Components[I] is TFrame then
            begin
             for j := 0 to TFrame(Owner.Components[I]).ComponentCount - 1 do
              if TFrame(Owner.Components[I]).Components[J] is tcontrol then

              begin
                R := TSingleResizer.Create(TFrame(Owner.Components[I]).Components[J] as TControl,
                                       Owner.Components[I] as TControl,
                                       FResizeFonts,
                                       FMinFontSize,
                                         FMaxFontSize);
               ResizerList.Add(R)
...
begin
     ...
     for I := 0 to ResizerList.Count - 1 do
         begin
          if TSingleResizer(ResizerList.Items[I]).Component is tframe then
          begin
